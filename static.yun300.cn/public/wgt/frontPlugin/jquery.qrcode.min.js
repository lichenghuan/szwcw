! function(u) {
    ...
    u.fn.qrcode = function(h) {
        "string" == typeof h ? h = {
            text: h
        } : h.text && (h.text = function(t) {
            var e, r, o, n;
            for (e = "", o = t.length, r = 0; r < o; r++) 1 <= (n = t.charCodeAt(r)) && n <= 127 ? e += t.charAt(r) : (2047 < n ? (e += String.fromCharCode(224 | n >> 12 & 15), e += String.fromCharCode(128 | n >> 6 & 63)) : e += String.fromCharCode(192 | n >> 6 & 31), e += String.fromCharCode(128 | n >> 0 & 63));
            return e
        }(h.text)), h = u.extend({}, {
            render: "canvas",
            width: 256,
        ...
        return this.each(function() {
            var t = "canvas" == h.render ? function() {
                var t = new l(h.typeNumber, h.correctLevel);
                t.addData(h.text), t.make();
                var e = document.createElement("canvas");
                e.width = h.width, e.height = h.height;
                for (var r = e.getContext("2d"), o = h.width / t.getModuleCount(), n = h.height / t.getModuleCount(), i = 0; i < t.getModuleCount(); i++)
                    for (var a = 0; a < t.getModuleCount(); a++) {
                        r.fillStyle = t.isDark(i, a) ? h.foreground : h.background;
                        var u = Math.ceil((a + 1) * o) - Math.floor(a * o),
                            s = Math.ceil((i + 1) * o) - Math.floor(i * o);
                        r.fillRect(Math.round(a * o), Math.round(i * n), u, s)
                    }
                return e
            }() : function() {
                var t = new l(h.typeNumber, h.correctLevel);
                t.addData(h.text), t.make();
                for (var e = u("<table></table>").css("width", h.width + "px").css("height", h.height + "px").css("border", "0px").css("border-collapse", "collapse").css("background-color", h.background), r = h.width / t.getModuleCount(), o = h.height / t.getModuleCount(), n = 0; n < t.getModuleCount(); n++)
                    for (var i = u("<tr></tr>").css("height", o + "px").appendTo(e), a = 0; a < t.getModuleCount(); a++) u("<td></td>").css("width", r + "px").css("background-color", t.isDark(n, a) ? h.foreground : h.background).appendTo(i);
                return e
            }();
            u(t).appendTo(this)
        })
        ...
        make: function() {
            if (this.typeNumber < 1) {
                var t = 1;
                for (t = 1; t < 40; t++) {
                    for (var e = v.getRSBlocks(t, this.errorCorrectLevel), r = new L, o = 0, n = 0; n < e.length; n++) o += e[n].dataCount;
        ...
        makeImpl: function(t, e) {
            ...
            this.setupPositionProbePattern(0, 0), this.setupPositionProbePattern(this.moduleCount - 7, 0), this.setupPositionProbePattern(0, this.moduleCount - 7), this.setupPositionAdjustPattern(), this.setupTimingPattern(), this.setupTypeInfo(t, e), 7 <= this.typeNumber && this.setupTypeNumber(t), null == this.dataCache && (this.dataCache = l.createData(this.typeNumber, this.errorCorrectLevel, this.dataList)), this.mapData(this.dataCache, e)
        },
        ...
        setupTypeInfo: function(t, e) {
            for (var r = this.errorCorrectLevel << 3 | e, o = p.getBCHTypeInfo(r), n = 0; n < 15; n++) {
                var i = !t && 1 == (o >> n & 1);
        ...
        mapData: function(t, e) {
            for (var r = -1, o = this.moduleCount - 1, n = 7, i = 0, a = this.moduleCount - 1; 0 < a; a -= 2)
                for (6 == a && a--;;) {
                    for (var u = 0; u < 2; u++)
                        if (null == this.modules[o][a - u]) {
                            var s = !1;
                            i < t.length && (s = 1 == (t[i] >>> n & 1)), p.getMask(e, o, a - u) && (s = !s), this.modules[o][a - u] = s, -1 == --n && (i++, n = 7)
                        }
        ...
        for (var o = v.getRSBlocks(t, e), n = new L, i = 0; i < r.length; i++) {
            var a = r[i];
            n.put(a.mode, 4), n.put(a.getLength(), p.getLengthInBits(a.mode, t)), a.write(n)
        }
        var u = 0;
        ...
        for (var r = 0, o = 0, n = 0, i = new Array(e.length), a = new Array(e.length), u = 0; u < e.length; u++) {
            ...
            r += s;
            var g = p.getErrorCorrectPolynomial(h),
                f = new C(i[u], g.getLength() - 1).mod(g);
            a[u] = new Array(g.getLength() - 1);
            for (l = 0; l < a[u].length; l++) {
                var d = l + f.getLength() - a[u].length;
                a[u][l] = 0 <= d ? f.get(d) : 0
            }
        }
        var c = 0;
    ...
    for (var o = {
            ...
            Q: 3,
            H: 2
        }, i = 0, a = 1, s = 2, h = 3, g = 4, f = 5, d = 6, c = 7, p = {
            ...
            getErrorCorrectPolynomial: function(t) {
                for (var e = new C([1], 0), r = 0; r < t; r++) e = e.multiply(new C([1, m.gexp(r)], 0));
                return e
            },
        ...
        }, t = 0; t < 8; t++) m.EXP_TABLE[t] = 1 << t;
    for (t = 8; t < 256; t++) m.EXP_TABLE[t] = m.EXP_TABLE[t - 4] ^ m.EXP_TABLE[t - 5] ^ m.EXP_TABLE[t - 6] ^ m.EXP_TABLE[t - 8];
    for (t = 0; t < 255; t++) m.LOG_TABLE[m.EXP_TABLE[t]] = t;
    ...
    C.prototype = {
        ...
        for (var o = r.length / 3, n = new Array, i = 0; i < o; i++)
            for (var a = r[3 * i + 0], u = r[3 * i + 1], s = r[3 * i + 2], h = 0; h < a; h++) n.push(new v(u, s));
        return n
